...
// TestExtendedCompatibility runs extended compatibility tests
func TestExtendedCompatibility(t *testing.T) {
	...
	t.Logf("Running %d extended test cases", len(testCases))
	
	// Group test cases by category for better reporting
	categories := make(map[string][]ExtendedTestCase)
...
func TestTokenizationProperties(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Description, func(t *testing.T) {
			...
			pretokens := sm.TokenizeWithStateMachine()
...
// BenchmarkExtendedCases benchmarks various categories of inputs
func BenchmarkExtendedCases(b *testing.B) {
	tokenizer, err := NewDefault()
	if err != nil || tokenizer.VocabSize() == 0 {
